...
interface B3PlatformEvidence {
  ...
  distribution: {
    ...
    { kind: 'development'; iosBuildNumber: string; signedIpaSha256: string; ipaEmbeddedAuthoritySha256: string; codeSigningCertificateSha256: string; installedBundleId: 'uk.eugnel.ks2spelling'; installedVersion: '0.3.0-b3'; installedBuild: string; installedEmbeddedAuthoritySha256: string; installedBuiltByDeveloper: true; sandboxReceiptVerified: true } |
...
`prepare:b3:distribution` writes clean-HEAD/fingerprint authority + xcconfig only. `B3SandboxProof` embeds exact values. Android flavour builds internal AAB. Verifier requires iOS mode exactly `development`, operator-provided signed IPA SHA, independently extracted IPA embedded authority values/SHA and IPA code-signing certificate SHA, installed bundle/version/build/embedded-authority equality, `devicectl`'s independent `builtByDeveloper: true` classification and sandbox receipt. It deliberately makes no installed-certificate equality claim because the supported Xcode 26.6 device interface does not expose that certificate. Android requires signed AAB SHA, independently extracted authority/versionCode, approved Play App Signing certificate, installer, and `pm path` pull/hash of ordered base then lexically ordered splits with installed embedded equality. No agent signing/keychain/provisioning.
...
15. iOS distribution is development only and binds signed IPA SHA, independently extracted authority/values and IPA cert, installed bundle/version/build/authority equality, and `devicectl` developer-app classification without claiming installed-certificate equality. Android binds signed AAB SHA/extracted authority/versionCode/Play cert/installer and ordered pulled base/split APK SHA multiset.
