...
## Canonical spelling audio boundary

Word, normal-sentence and slow-sentence audio is generated by the controlled
content-authoring pipeline from voices whose provenance and redistribution
rights are recorded. The exact Starter 20 audio inventory is an installed
application resource covered by the distribution signature and verified
against its compiled per-asset size and SHA-256 authority before playback.
Downloaded or paid spelling audio remains signed pack data, verified before
atomic installation. Once the required local audio is available, spelling
practice and replay work without a network connection.

The application does not generate spelling audio at runtime. Web Speech,
`AVSpeechSynthesizer`, Android `TextToSpeech` and network speech providers must
not be used as an automatic or silent fallback. A missing, corrupt or
unavailable asset produces an explicit recoverable audio error; it does not
change learner progress, invent an answer or substitute device-dependent
pronunciation.

VoiceOver and TalkBack remain supported through ordinary semantic
accessibility. Their user-controlled screen reading is not a spelling-content
audio source. A future, separately reviewed accessibility feature may permit
ephemeral on-device reading of the currently displayed word or sentence, but
it must remain non-canonical, optional, offline-only, excluded from scoring and
must never be saved, repackaged or sold as pack audio.
...
Physical accessibility and performance certification, release signing, store
records and final store metadata remain deferred to the final release proof.
Virtual devices and hosted device services may advance development and
compatibility work, but public store submission remains blocked until Release
Commerce Certification has exercised signed final-candidate builds on
accessible physical or approved hosted-real devices. Passing a narrower test
or unsigned build must not be described as production or store readiness.
