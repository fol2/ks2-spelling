...
Category roles use GitHub's existing `bug` and `enhancement` labels, which already match the canonical names. `wontfix` already exists in this tracker. Create the four missing state labels once before the first `/triage` run:
