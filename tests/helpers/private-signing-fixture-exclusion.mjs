...
// A generated root needs two complete, matching snapshots, and each snapshot
// is read twice to close the collect/read race. Keep that work budget separate
// from the packageable-surface ceiling so every permitted generated byte can
// actually be verified without weakening the 4 GiB surface limit.
const MAX_RAW_WORK_BYTES = MAX_TOTAL_BYTES * 4;
...
const DEFAULT_SCAN_LIMITS = Object.freeze({
  maxRawBytes: MAX_RAW_WORK_BYTES,
