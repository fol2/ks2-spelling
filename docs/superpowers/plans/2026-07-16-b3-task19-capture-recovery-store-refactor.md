...
Publication is: create the UUID temporary with `O_EXCL|O_NOFOLLOW`, write the exact canonical bytes in a checked partial-write loop, sync and close it, sync the installing parent, create-only hard-link it to the common target, sync the target parent, validate an exact strict readback, then remove only recognised temporaries whose filename carries the same exact command hash and target kind after descriptor/inode/type/mode/link-count checks, and sync the installing parent again. A stable fully written temporary whose exact bytes equal the expected target may be adopted while holding the global lock. An incomplete regular single-link temporary is permitted only when its stable bytes are an exact prefix, including the empty prefix, of that filename kind's exact canonical target; it is untrusted crash debris and never authority. The core therefore uses one module-owned, read-only expected-bytes resolver to derive the exact canonical bytes for every recognised filename hash/kind; it must never treat an unrelated single-link entry as valid merely because its inode policy is valid.

The resolver follows one immutable acyclic authority graph: `recovery-owner <- globally allocated/reachable restart-required predecessor + signed build authority`; `archive-manifest <- validated owner + strict allow-listed archived payload members`; `archive-authority <- validated owner + independently derived manifest + signed build authority`; `recovery-terminal <- validated owner + independently derived archive authority`. It may read only those predecessor/build/payload roots. It must not use the candidate installing bytes, the target currently being validated or a digest derived from either as expected-value input; it must not mutate the filesystem or re-enter the claim installer. Thus a hostile candidate and target containing identical self-consistent bytes still reject unless the independent graph derives those bytes. The production facade owns this resolver capability and its production methods accept no resolver, lock coordinator or storage adapter option. Only the internal core test factory may inject a resolver, and source-contract tests require the production core import to remain confined to the facade while public-option poison objects are ignored or rejected before mutation. Unallocated/unreachable authority, an unprovable expected value, non-prefix bytes, wrong types/modes, unrecognised or external extra links, unexpected names, excessive global entries or excessive target-scoped debris reject before target or terminal publication. The helper may inspect and count other command/kind entries for namespace policy but must never adopt, unlink or otherwise alter them.
...
Loop stages are parameterised over every checkpoint and derived member. This logical list is not by itself the exhaustive crash claim. The core storage adapter must emit a closed durability trace entry for every durable primitive with `{ operation, item, primitive }`; `operation` is exactly `install-recovery-claim`, `item` is `recovery-installing | recovery-owner | archive-manifest | archive-authority | recovery-terminal`, and `temporary-write-progress` additionally carries the positive safe-integer byte `offset`. Bootstrap may emit only `directory-created` and `directory-synced` for item `recovery-installing`. After the short acquisition child's direct `close` reports exit `0`, no signal and fully drained bounded stderr while the parent still holds the shared directory file description, emit `{ operation: 'install-recovery-claim', item: <target-kind>, primitive: 'kernel-lock-acquired' }`; no coordination event is emitted for an acquisition failure. After the critical section, close the retained parent `FileHandle` and emit the corresponding `kernel-lock-released`, then return or rethrow the storage result. Parent-death traces end without released; the waiting helper's later acquired event is the release proof. The closed acquisition-failure table is descriptor validation failure, spawn error, exit `75`, other non-zero exit, signal, stderr overflow and watchdog termination. Every row proves the child was absent or reaped, the parent handle closed exactly once, an independent descriptor acquired immediately, the complete post-bootstrap namespace snapshot remained unchanged and no post-bootstrap durable trace was emitted. A separate ordering test emits `exit(0)` before late stderr and `close`, proves no acquired event or critical-section entry at `exit`, then proves overflow rejects or bounded EOF plus `close(0)` succeeds. Durable `primitive` is one of:
...
The test also asserts that every logical stage emitted its required primitive set and that no production storage write/link/rename/unlink/fsync path bypasses the injected traced storage adapter. Separate barrier tests replace or race a source during stable validation and during each move. The duplicate-helper schedule suite independently covers `recovery-owner`, `archive-manifest`, `archive-authority` and `recovery-terminal`. For every kind it captures descriptor owner and waiting-helper traces, injects public-helper process exit after every role-specific durable primitive, proves descriptor-close/kernel release, waiting-helper exclusion, same-kind target-scoped cleanup and same/different-kind serialisation, and requires the waiting helper or a fresh helper to converge. It separately proves acquisition-timeout failure is post-bootstrap mutation-free while another public helper deliberately retains the directory descriptor. Adding a future durable primitive automatically expands the reachable-state and contention crash tables.
...
Production code must not accept `afterArchive`, `afterSuccessorCurrentRead`, `beforeClear`, `beforeClearCommandRead` or equivalent hooks. Recovery storage fault/barrier injection exists only in the internal core test factory, and generic-consumption successor-link pausing exists only in the separate internal issued-ledger test factory; runtime callers cannot select either. The production exports of both modules expose neither factory nor any injectable callback/options path, and a source-contract test proves production modules cannot import or reach them. The public facade is the only production import of the core. Direct application imports of issued-command recovery helpers or abandoned-capture helpers fail a source-contract test. A RED source/behaviour contract also proves the generic `transitionB3IssuedCommand()` rejects both old recovery transition pairs and can never create `state-restart-executing`, `state-restart-complete` or `successor-restart-executing`; only the store may validate embedded records and publish owner/terminal authority.
...
Write RED tests for the closed owner, terminal and generic-consumption schemas, domain hashes, owner/terminal/archive triple binding, create-only equality/conflict, terminal-without-archive, archive-without-terminal and generic clear rejection. The generic table covers all eight allowed source states; `restart-required`, both embedded recovery states and an unknown state reject before the internal barrier. Race a paused generic clear against an ordinary transition from the same source in both winner orders, prove the losing clear has no external side effect, converge two identical clears, preserve a later command while a stale clear converges, reject generic-consumption beside a recovery terminal, reject same-command-hash reuse after generic consumption and reject every stale `.consumed.json`. Crash/restart generic publication at temporary create, partial/complete write, temporary sync, successor link, target-parent sync and temporary cleanup; every gap derives one consumed command, repairs only its exact alias and permits a distinct next allocation. Race an ordinary `restart-required -> launched` successor against recovery ownership and prove exactly one can occupy `successor-restart-required`. Prove that owner embeds the sole executing record and terminal embeds the sole restart-complete record, leaving no unenumerated recovery state/successor write.

At the installing seam, table all four kinds `recovery-owner | archive-manifest | archive-authority | recovery-terminal` against an allocated/reachable exact valid prefix, an unallocated or unprovable filename authority, a non-prefix candidate and a hostile candidate/target pair containing the same independently invalid bytes. Prove the immutable resolver DAG, no mutation/core re-entry, public-option poison rejection, production-import confinement, exact private modes, allocation-chain reachability, target-scoped cleanup and unrelated valid-prefix coexistence. The macOS fd-mode `lockf` table covers exact binary/argv/environment/fd mapping, descriptor/path identity, bounded stderr/timeout, acquisition-child reap, persistence after child exit, normal close and kernel release after public-helper death. An internal core-only injected lock coordinator and resolver supply deterministic R2/R5 unit tests on any host; production exports and the public facade cannot select either. A separate Darwin integration test exercises real `/usr/bin/lockf`, while non-Darwin production-path tests require fail-closed zero mutation.
