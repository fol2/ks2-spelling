...
function finalSize(slot, companion, stage) {
  ...
  const painted = Number((slot.size * scale).toFixed(2));
  // Far-lane small species can otherwise hit the same global minimum at every
  // stage and appear never to grow. Preserve visible evolution without letting
  // foreground artwork exceed the established maximum.
  const stageFloor = 23 + stage * 0.75;
  return Number(Math.max(stageFloor, Math.min(49, painted)).toFixed(2));
}
