...
export function parsePackagedAndroidPermissions(output, options = {}) {
  ...
  if (
    ...
    throw packagedPermissionError(
      `Packaged Android permission surface differs from exact biometric, network and Play Billing authority; declared=${declaredPermissions.join(',')}; requested=${requestedPermissions.join(',')}`,
    );
  }
