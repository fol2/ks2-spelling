...
const RUNBOOK_PATH = join(
  ROOT,
  'docs/operations/2026-08-15-submission-day-runbook.md',
);
...
test('the submission-day go/no-go table treats backup removal as a hard stop and the iCloud replica as not', async () => {
  const runbook = await readFile(RUNBOOK_PATH, 'utf8');
  const hardStops = headingBlock(runbook, '### Hard stops');
  const notHardStop = headingBlock(runbook, '### Not a hard stop');

  assert.match(hardStops, /https:\/\/github\.com\/fol2\/ks2-spelling\/issues\/198/);
  assert.match(hardStops, /Learning backup gone from the RC/);
  assert.doesNotMatch(hardStops, /issues\/199/);

  assert.match(notHardStop, /https:\/\/github\.com\/fol2\/ks2-spelling\/issues\/199/);
  assert.doesNotMatch(notHardStop, /issues\/198/);
  assert.doesNotMatch(notHardStop, /issues\/187/);

  assert.doesNotMatch(runbook, /187[\s\S]{0,500}notes adapt/i);
  assert.doesNotMatch(runbook, /notes adapt[\s\S]{0,500}187/i);
});
